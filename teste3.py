...
class MLP:
    ...
    def train(self, X, y, X_test, y_test, epochs, learning_rate):
        ...
        for epoch in range(epochs):
            ...
            train_error = np.mean(np.abs(y - self.output))
            self.loss_train.append(train_error)

            self.forward(X_test)
            test_error = np.mean(np.abs(y_test - self.output))
            self.loss_test.append(test_error)
...
plt.plot(range(epochs), mlp.loss_train, 'r-', label='Train Loss')
plt.plot(range(epochs), mlp.loss_test, 'b-', label='Test Loss')
plt.xlabel('Epochs')
plt.ylabel('Loss')
